...
const Genders = () => {
    ...
    // Load genders
    const loadGenders = async (page = 1) => {
        setLoading(true);
        setError('');
        try {
            const params = {
            ...
            const response = await genderService.getAllGenders(params);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            resetForm();
            loadGenders(pagination.currentPage);
        } catch (err) {
    ...
    const handleDelete = async (id) => {
        ...
        try {
            ...
            setError(err.message || 'Failed to delete gender');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleToggleStatus = async (id) => {
        ...
        setError('');
        try {
            await genderService.toggleGenderStatus(id);
            setSuccess('Gender status updated successfully');
            loadGenders(pagination.currentPage);
        } catch (err) {
            setError(err.message || 'Failed to update gender status');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="w-full">
            ...
            <div className="mb-6 ">
                ...
                <p className="text-sm text-gray-600">
                    Manage genders with their names, descriptions, and status.
                </p>
            </div>
                                ...
                                genders?.map((gender) => (
                                    <tr key={gender?._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                            {new Date(gender?.createdAt).toLocaleDateString()}
                                        </td>
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div className="flex items-center justify-end gap-2">
                                                <button
                                                    onClick={() => handleToggleStatus(gender?._id)}
                                                    className={`p-1 rounded cursor-pointer ${
                                                        gender?.status === 'active' 
                                                            ? 'text-red-600 hover:bg-red-50' 
                                                            : 'text-green-600 hover:bg-green-50'
                                                    }`}
                                                    title={gender?.status === 'active' ? 'Deactivate' : 'Activate'}
                                                >
                                                    {gender?.status === 'active' ? (
            ...
            <Modal open={showModal} onClose={handleModalClose}>
                <Box sx={modalStyle} component="form" onSubmit={handleSubmit}>
                    ...
                    <Alert severity="info" sx={{ mb: 3 }}>
                        This form manages gender configurations and is secured in compliance with HIPAA regulations.
                    </Alert>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, mt: 3 }}>
                        ...
                        <Button
                            ...
                            disabled={loading}
                        >
                            {loading ? 'Saving...' : (editingGender ? 'Update Gender' : 'Add Gender')}
                        </Button>
                    </Box>
                </Box>
            </Modal>
